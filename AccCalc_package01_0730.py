...
#### Ref - reference alele
#### Alt - alternative alele
#### WT - wild type phenotype
#### MUT - mutant phenotype
#### Acc - accuracy


#Import packages used for packages functions
import pandas as pd
import numpy as np
...
## Import hapmap file into a numericaly coded table in the Pandas (DataFrame),  
...
## area: alow to select only small part from the vcf file
#### list of integers with the genomic area definition in format [chromosome,start position,stop position]
...
##################################################################################################
## Import vcf file into a numericaly coded table in the Pandas (DataFrame),  
### table format suitable for the other functions in the package
######## 0 for Ref (0/0), 1 for Alt, -2 for missing and heterozygotes
## area: alow to select only small part from the vcf file
#### list of integers with the genomic area definition in format [chromosome,start position,stop position]
## alt: define the coding for Alt, in case more than one Alt variant
#### all - all Alt's coded as Alt(1), max - only the most common Alt as Alt, 1st - only the first (1/1) Alt as Alt
## het: define the coding for heterozygosity
...
## Import phenotype file into a numericaly coded table in the Pandas (DataFrame),  
### table format suitable for the other functions in the package
######## 0 for WT, 1 for MUT, -2 for missing and other phenotypes
## wt/mut: list of phenotypes used as WT/MUT 
### ect. ["B","Br"] for categerical or [1,10] for quantitative (first number is included lower limit and the second is unincluded upper limit)
...
## sep: type of separotor in phenotype file
## col_n: column number in phenotype file, counted from 0 (sample names in first colum)
def imp_phen_tab(file,wt,mut,phen_type="categ",Head=True,sep="t",col_n=1):
    ...
    for line in file:
        ...
        if Head and n==0:n=1
            ...
            hd.append(line[0])
            if line[col_n] in wt: phn.append(0)
            elif line[col_n] in mut: phn.append(1)
            else: phn.append(-2)
    phen_tab=pd.DataFrame([phn],columns=hd)
    return phen_tab
##################################################################################################
## Create Synthetic phenotype from selected genotype line in vcf file
## return Phenotype table with numericaly coded phenotype info (DataFrame),  
### table format suitable for the other functions in the package
######## 0 for WT, 1 for MUT, -2 for missing and other phenotypes
## chrom/pos numeric specification for line selection
## wt/mut: list of phenotypes used as WT/MUT ect. ["0/0","0/1"]
...
## pos: integer value of position of selected locus
...
def synthetic_phen_from_tab(gen_tab,chrom,pos,wt,mut):
    ...
    return phen_tab
#####################################################################################################
## Write a phenotype txt file prom pheno_tab
...
def acc_cal(gen_tab, phen_tab, cal="A", flip="off"):
    if list(gen_tab.columns.values)[9:]==list(phen_tab.columns.values):
        ...
        for n in range(0,len(gen_tab.index.values)):
             ...
             if cal=="A":
                 ref_phn_val="WT"
                 if (AD)!=0 and (BC): accr=((A/AD)+(B/BC))/2*100
                 else: accr=0
                 accp=((A/n_wt)+(B/n_mut))/2*100
                 if (ABCD)!=0: accrc=(A+B)/(ABCD)*100
                 else: accrc=0
                 accpc=(A+B)/l_all*100
                 if flip=="on" and accr<50:
                     if (AD)!=0 and (BC):accr=((D/AD)+(C/BC))/2*100
                     else: accr=0
                     accp=((D/n_wt)+(C/n_mut))/2*100
...
#####################################################################################################
## Sort phen_tab into a sample order acording the gen_tab
### Necesary for Accurycy calculation
...
### pval_file: name of file with p-values in formate .txt or .csv
### file_sep: type of column separator in pval_file
### pval_pos: position of coulumn with the p-values in pval_file (start from 0)
### chrom_pos: position of coulumn with the chromosome in pval_file (start from 0)
### pos_pos: position of coulumn with the position in pval_file (start from 0)
def add_pval(acc_tab,pval_file,file_sep=",",pval_pos=3,chrom_pos=0,pos_pos=1):
    ...
    acc_tab["-log10pval"]=-np.log10(acc_tab["p-value"])
    acc_tab["-log10pval"]=pd.to_numeric(acc_tab["-log10pval"] ,errors='coerce')
    return acc_tab

#####################################################################################################   
## Add distance from position into Accu_tab
### point: position for caounting distance
####### max_pval - automaticaly set as a position with the biggest -log10 pval
...
def add_distance(acc_tab,point="max_pval"):
    ...
    acc_tab["dist_to_point"]=pd.to_numeric(dist_list ,errors='coerce')
...
def plot_accuracy(acc_tab):
    ...
    if len(chromosomes)>1:
        ...
        for accuracy in ['Avr_acc(%)','Avr_acc_pes(%)','Comb_acc(%)','Comb_acc_pes(%)']:
            ...
            plt.colorbar(ScalarMappable(norm=norm, cmap=cmap),orientation='vertical',label=accuracy)
            cmap = LinearSegmentedColormap.from_list("name",colors=["gainsboro","gainsboro","silver","silver","lightblue","blue","navy"])
            plt.colorbar(ScalarMappable(norm=norm, cmap=cmap),orientation='vertical')
