...
def hmpline_to_tab (line):
    ...
    try: 
        ...
        hmpchrom=""
        for v in line[2]:
            if v.isdigit(): hmpchrom=hmpchrom+v
    if hmpchrom!="" :
        ln=[int(hmpchrom),line[3],line[0],allele[0],allele[1],"NA","NA",inf,"NA"]
    else:
        ln=[0,line[3],line[0],allele[0],allele[1],"NA","NA",inf,"NA"]
...
def plot_accuracy(acc_tab):
    chromosomes=list(np.unique(acc_tab["#CHROM"]))
    print(chromosomes)
    ...
    if len(chromosomes)>1:
        ...
        for accuracy in ['Avr_acc(%)','Avr_acc_pes(%)','Comb_acc(%)','Comb_acc_pes(%)']:
            ...
            fig.colorbar(ScalarMappable(norm=norm, cmap=cmap),orientation='vertical', label=accuracy,ax=ax,s=2)
